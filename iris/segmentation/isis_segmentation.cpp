...
void isis::segmentation(const cv::Mat& source, segmentation_t& segmentation)
{
    ...
    cv::Mat limbusCroppedGray = preprocessed(limbus.getbbox());
    
    // find pupil brute force
    auto pupil = findPupilBF(limbusCroppedGray, limbus);
...
void isis::preprocess(cv::Mat& source, cv::Mat& out, const int& reflectionFilterMode, const int& finalSize)
{
    ...
    cropEye(source, source, isis::HAARCASCADE_EYE_TREE_EYEGLASSES);
...
isis::Circle isis::findLimbusBF(const cv::Mat& grey)
{
    ...
    auto tmp = std::vector<std::pair<Circle, double>>();
    for(int k = 1; k < 18; k++)
    {
        cv::Mat posterized;
        ...
        findCircles(posterized, circles, 35, 90);
...
isis::Circle isis::findPupilBF(const cv::Mat& grey, const Circle& bestLimbus)
{
    ...
    for(int k = 1; k < 18; k++)
    {
        ...
        std::vector<Circle> circles;
        findCircles(posterized, circles, 8, 35);
        //TODO: da ottimizzare
...
void isis::findCircles(const cv::Mat& mat, std::vector<Circle>& outputCircles, const double& minRadius, const double& maxRadius)
{
